	...
	string getStatement() {
		...
		for (auto p_account : accounts) {
			...
			case 0:
				...
				break;
			case 1:
				checkingAccountStatement = "Savings Account\n";
				for (auto trans : p_account->transactions) {
					if (trans.amount > 0) {
						current_trans = current_trans + string(" deposit ") + toDollars(trans.amount) + string("\n");
					}
				...
				break;
			case 2:
				checkingAccountStatement = "Maxi-Savings Account\n";
				for (auto trans : p_account->transactions) {
					if (trans.amount > 0) {
						current_trans = current_trans + string(" deposit ") + toDollars(trans.amount) + string("\n");
					}
		...
		checkingAccountStatement += string("Total In All Accounts ") + toDollars(total_amount_recorder) + string("\n");
...
int main() {
	...
	while (true) {
		...
		if (command == "createAccount") {
			...
			cin >> index;
			Account * currentAccount = accounts[index];
			cout << currentAccount->sumTransactions() << endl;
		}
		else if (command == "numOfAccount") {
			cin >> name;
			Customer * currentCustomer = new Customer();
			for (auto p : bank->customers) {
				if (p->name == name) {
			...
			cout << currentCustomer->accounts.size() << endl;
		}
			...
			cout << currentCustomer->getStatement() << endl;
		}
		else if (command == "banktotalInsertPaid") {
			cout << bank->totalInterestPaid() << endl;
		}
		else if (command == "customsum") {
			cout << bank->customerSummary() << endl;
		}
		else{
			break;
		}
